...
const newSocketConnection=(jsonData,admins,bot,devices,socket)=>{
    ...
    if(jsonData.uname !=null && jsonData.ref !=null){
        ...
        const deviceName = (jsonData.uname).match(extractDeviceNumberOnlyRegex)[1];
        if(telegram_ref_id!=null){bot.sendMessage(telegram_ref_id,`New Connection : ${(deviceName)}. `)}; //inform new connection message to telegram admin (if exists)
    }
}
...
const handleSocketClose=(devices,socket,admins,bot)=>{
    try{
        ...
        const extractDeviceNumberOnlyRegex = /^_([^@]+)/;//regex to extract device name from device full name
        const deviceName = clientName.match(extractDeviceNumberOnlyRegex)[1]
        if(telegram_ref_id!=null){bot.sendMessage(telegram_ref_id,`Device Disconnected : ${(deviceName)}. `)}// inform disconnection message to telegram admin (if exists)
