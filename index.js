...
const blacklisted = []; //stores blacklisted telegram ids


//custom imports
const passportRunner = require("./Resources/Auth"); //import google signin auth js (custom)
const Routes = require("./Resources/Route");//all routes
...
app.use('/',Routes(passport,isLoggedIn,userSchema,admins,devices,blacklisted)); //routes setup
...
bot.on('callback_query', (query) => {
    ...
    console.log(admins)
    let clientSelectionRegex = /_(\w+)@(\w+)/; 
    if(clientSelectionRegex.test(data)){
        if(devices[data] && devices[data].ref === admins[chatId].uname){
            admins[id].ref = data; //set selected client as admin ref
            const extractDeviceNumberOnlyRegex = /^_([^@]+)/;
            const device = data.match(extractDeviceNumberOnlyRegex)[1]
            bot.sendMessage(chatId, `Selected ${device}`);
        }else{
            bot.sendMessage(chatId,`Device not available or You dont have access!`)
        }
    }else{
