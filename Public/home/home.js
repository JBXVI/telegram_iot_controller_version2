...
const getData=async()=>{
    ...
    if(response.data.success === true){
        ...
        for(i=1;i<=(data.clientTokens).length;i++){
            devices.innerHTML+=`<div  class="input-group mb-3"><span class="input-group-text groupTitle" onclick="copyToken('${data.clientTokens[i-1].token}')">Client${i}</span><input type="text" class="form-control tokenInput"  aria-describedby="basic-addon3" value="${data.clientTokens[i-1].token}" disabled ></div>`
        }
